class IntervalTimerDisplay {
    constructor(intervalData, containerId) {
        ...
        this.largeRoadmapScaleX = d3
            .scaleLinear()
            .domain([0, numSpaces - 1])
            .range([
                this.largeRoadmapMargin,
                this.largeRoadmapWidth - this.largeRoadmapMargin,
            ]);

        this.largeRoadmapSvg = d3
            .select("#roadmap-viz-container")
            .append("svg")
            .attr("width", 0)
            .attr("height", 0);
        ...
        this.smallRoadmapScaleX = d3
            .scaleLinear()
            .domain([0, this.data.intervalPattern.length / 2 + 1])
            .range([0, this.smallRoadmapWidth]);

        const gapWidth =
            this.smallRoadmapScaleX(1) -
            this.smallRoadmapScaleX(0) -
            2 * this.smallRoadmapDotR;

        this.smallRoadmapScaleWidth = d3
            .scaleLinear()
            .domain([0, 1])
            .range([0, gapWidth]);

        this.smallRoadmapSvg = d3
            .select("#small-roadmap-viz-container")
            .append("svg")
            .attr("width", 0)
            .attr("height", 0)
    ...
    setStartEndTime() {
        ...
        const endDate =
            Date.now() +
            (this.data.workoutLength - this.data.totalTimeElapsed) * 1000;
        const end = new Intl.DateTimeFormat("en-US", formatOptions).format(
            endDate
        );
        d3.select("#top-roadmap")
            .selectAll(".clock-time")
            .data([start, end])
            .text((d) => d);
    }
    ...
    resize() {
        ...
        this.largeRoadmapDotR = Math.min(
            this.largeRoadmapDotR,
            (0.8 * this.largeRoadmapWidth) /
                (2 * this.data.mainWorkoutList.length)
        );
    ...
    update() {
        ...
        if (!this.data.warmUpComplete) {
            workoutName =
                '<u style="font-size: min(8vw, 8vh);">Warm Up:</u><br>' +
                workoutName;
        }
        if (this.data.mainWorkoutComplete() && !this.data.stretchComplete) {
            workoutName =
                '<u style="font-size: min(8vw, 8vh);">Stretch:</u><br>' +
                workoutName;
        }
        ...
        this.containerSelect.select("#name-container").html(workoutName);
        ...
        let timeString = "";
        if (timeRemaining >= 60) {
            timeString = `${dateObj.getMinutes()}:${(
                "0" + dateObj.getSeconds()
            ).slice(-2)}`;
        } else {
            timeString = timeRemaining.toFixed(1);
        }
        this.containerSelect.select("#countdown-time").text(timeString);
    ...
    updateLargeRoadmap() {
        ...
        const numBreaks = Math.floor(
            this.data.mainWorkoutList.length / this.data.countPerPass
        );
        let breaks = [];
    ...
    updateSmallRoadmap() {
        ...
        this.smallRoadmapSvg
            ...
            .attr(
                "cx",
                (d, i) => this.smallRoadmapScaleX(i) + this.smallRoadmapDotR
            )
            .attr("cy", midY)
        ...
        this.smallRoadmapSvg
            ...
            .attr(
                ...
                (d, i) =>
                    `translate(${
                        this.smallRoadmapScaleX(i) + this.smallRoadmapDotR
                    }, ${midY})`
            )
            .style("fill", "black")
            .classed("dot", true);
        ...
        this.smallRoadmapSvg
            .selectAll(".rest-line-incomplete")
            .data(restState)
            .join("rect")
            .attr(
                "x",
                (d, i) => this.smallRoadmapScaleX(i) + 2 * this.smallRoadmapDotR
            )
