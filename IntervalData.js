class IntervalData {
    ...
    update(deltaTime) {
        ...
        this.totalTimeElapsed += deltaTime * this.speedup;
    ...
    timeRemaining() {
        ...
        if (!this.mainWorkoutComplete()) {
            ...
            if (
                this.workoutIndex % this.countPerPass ===
                    this.countPerPass - 1 &&
                this.intervalIndex === this.intervalPattern.length - 1
            ) {
    ...
    static BuildData(
        ...
        if (warmUpOptions) {
            while (warmUpList.length < 6) {
                let thisWarmup = warmUpOptions.splice(
                    Math.floor(Math.random() * warmUpOptions.length),
                    1
                )[0];
        ...
        if (stretchOptions) {
            while (stretchList.length < 6) {
                let thisStretch = stretchOptions.splice(
                    Math.floor(Math.random() * stretchOptions.length),
                    1
                )[0];
        ...
        // MAIN WORKOUT
        if (intervalPattern) {
            outData.intervalPattern = intervalPattern
                .split(",")
                .map((d) => parseInt(d));
        } else {
            ...
            const index = Math.floor(
                Math.random() * defaultIntervalPatterns.length
            );
            outData.intervalPattern = defaultIntervalPatterns[index];
        }

        const singlePassIntervalLength = outData.intervalPattern.reduce(
            (x, y) => x + y
        );
        outData.countPerPass = Math.max(
            1,
            Math.floor((15 * 60) / singlePassIntervalLength)
        );

        let workoutCount = outData.countPerPass * bigIntervalCount;
        let workoutTypes = [...workoutOptions.keys()].filter(
            (type) => type !== "warm-up" && type !== "stretch"
        );
        let workoutList = [];
        let typeIndex = 0;
        while (workoutList.length < workoutCount) {
            ...
            let thisWorkout = options.splice(
        ...
        outData.workoutLength =
            outData.intervalPattern.reduce((x, y) => x + y) *
            workoutList.length;
